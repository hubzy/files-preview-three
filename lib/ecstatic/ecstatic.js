...
const renderMd = require('./ecstatic/render-md');
const version = '3.2.3';
...
const UrlExt = [
  'vue@v2.6.14.js',
  'download.min@v4.21.js',
  'vs-toast.min@1.4.0.js',
  'viewer.min@1.10.1.js',
  'three.min.js',
  'OrbitControls.js',
  'GLTFLoader.js',
  'RGBELoader.js',
...
module.exports = function createMiddleware(_dir, _options) {
  ...
  return function middleware(req, res, next) {
    ...
    */
    ...
    function statFile() {
      // const UrlfileName = req.url.split('/').pop();
      // if (UrlExtName.indexOf(UrlfileName) != -1) {
      //   const nameArr = UrlfileName.split('.');
      //   const ext = nameArr.pop();
      //   let pathName = req.url.indexOf('template') != -1 ? 'ecstatic/show-dir/template/' :
      //     req.url.indexOf('indexMd') != -1 ? 'ecstatic/render-md/template/' : 'resources/';
      //   return fs.readFile(path.join(__dirname, pathName + UrlfileName), 'utf8', (Error, Data) => {
      //     if (!Error) {
      //       res.writeHead(200, {
      //         'Content-Type':
      ...
      //   })
      // }
      let UrlfileName = req.url.split('/').pop();
      if (UrlExt.indexOf(UrlfileName) != -1) {
        fs.readFile(path.resolve(__dirname, 'resources/'+UrlfileName), 'utf8', (Error, Data) => {
          if (Data) {
      ...
      if (req.url.indexOf('template.css') != -1) {
        fs.readFile(path.resolve(__dirname, 'ecstatic/show-dir/template/template.css'), 'utf8', (cssError, cssData) => {
          if (cssData) {
      ...
      if (req.url.indexOf('template.js') != -1) {
        fs.readFile(path.resolve(__dirname, 'ecstatic/show-dir/template/template.js'), 'utf8', (cssError, Data) => {
          if (Data) {
            res.writeHead(200, {'Content-type':'application/javascript; charset=utf-8'});
            res.setHeader('cache-control', 'max-age=3600');
            res.end(Data);
      ...
      if (req.url.indexOf('indexMd.css') != -1) {
        fs.readFile(path.resolve(__dirname, 'ecstatic/render-md/template/indexMd.css'), 'utf8', (cssError, cssData) => {
          if (cssData) {
            res.writeHead(200, { 'Content-Type': 'text/css; charset=utf-8' });
            res.setHeader('cache-control', 'max-age=3600');
            res.end(cssData);
      ...
      if (req.url.indexOf('viewer.min@1.10.1.css') != -1) {
        fs.readFile(path.resolve(__dirname, 'resources/viewer.min@1.10.1.css'), 'utf8', (cssError, cssData) => {
          if (cssData) {
            res.writeHead(200, { 'Content-Type': 'text/css; charset=utf-8' });
            res.setHeader('cache-control', 'max-age=3600');
            res.end(cssData);
      ...
      if (req.url.indexOf('favicon.ico') != -1) {
        fs.readFile(path.resolve(__dirname, 'resources/favicon.ico'), (Error, Data) => {
          if (Data) {
      ...
      fs.stat(file, (err, stat) => {
        if (err && (err.code === 'ENOENT' || err.code === 'ENOTDIR')) {
          ...
          if (contentType == 'text/markdown' && req.query.original != '1') {
            // 渲染MD
            // renderMd(opts, stat)(req, res)
            _renderMd(req, res)
            return
          } else {
            serve(stat);
